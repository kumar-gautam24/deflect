...
from deflect_common.llm.base import Completion, get_client
...
def test_unknown_provider_is_rejected():
    with pytest.raises(ValueError, match="unknown provider"):
        get_client(provider="anthropic", model="whatever")


def test_ollama_client_needs_no_credential():
    """The local provider must be usable without a key, which is what makes it a
    working fallback rather than a talking point."""
    client = get_client(provider="ollama", model="llama3", base_url="http://localhost:11434")
